...
from src.components.dialog_attendance import attendance_result_dialog
...
def teacher_tab_take_attendence():
    ...
    selected_subject_id = subject_option.get(selected_subject_label)
    with col2:
    ...
    with c2:
        ...
        if st.button("Run Face Analysis", width='stretch',type="secondary", icon=":material/analytics:",disabled=not has_photos):
            with st.spinner("Scanning the classroom photos.."):
                all_detected_id = {}
                for idx , img in enumerate(st.session_state["attendance_images"]) :
                ...
                enrolled_res = supabase.table('subject_sutdents').select("*, students(*)").eq("subject_id", selected_subject_id).execute()  
...
def login_teacher(username,password):
    if not username or not password:
        return False
    teacher = teacher_login(username=username, password=password)
    if teacher:
...
def teacher_screen_login():
    ...
    with btnc2:
        clicked = st.button("Register Instead", type="primary", width="stretch")


        if clicked:
            
            st.session_state.teacher_login_type = "register"
            #st.write("state now =", st.session_state.teacher_login_type)
            st.rerun()
...
def register_teacher(teacher_username, teacher_name, teacher_password, teacher_pass_conf):
    ...
    try:
        create_teacher(username=teacher_username ,password=teacher_password,name=teacher_name)
...
def teacher_screen_register():
    ...
    st.space()  
    teacher_username = st.text_input("Enter username", placeholder="ananyaroy").strip()
    teacher_name = st.text_input("Enter name", placeholder="Dr.Ananya Roy").strip()
    
    teacher_password= st.text_input("Enter password", type="password", placeholder="Enter password").strip() 
    teacher_pass_conf= st.text_input("Confirm password", type="password", placeholder="Confirm password").strip()
    st.divider()
    btnc1,btnc2=st.columns(2)
    with btnc1:
