...
def student_dashboard():
    ...
    for i , sub_node in enumerate(subjects):
        ...
        def unenroll_btn():
                if st.button("Unenroll from this course", type="tertiary", icon=":material/delete_forever:"):
...
def student_screen():
     ...
     if photo_source:
         ...
         with st.spinner("AI is Scanning..."):
             detected,all_ids, nump_faces=predict_attendance(img)
            
             if nump_faces ==0:
                 st.warning("Face not found")

             elif nump_faces>1:
                 st.warning("Multiple facefs found")
             else:
                 if detected:
                     ...
                     if student:
                         ...
                         st.info("Face not recognized, YOu are a new Student")
                         show_registration=True
     if show_registration:
         with st.container(border=True):
             st.header("Registe new Profile")
             new_name = st.text_input("Enter your name", placeholder="E.g Amitabh Bachan")
             st.subheader("Optional:Voice Enrollment")
             st.info("Enroll your voice only for attendence")
             audio_data=None
             try:
                 audio_data = st.audio_input("Record a short phrase saying I am present , My name is Akash")
             except Exception:
